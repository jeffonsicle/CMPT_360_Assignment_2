...
# 3. Build
In order to run the program you need to type 'make' in the command line, then type ./sched --policy=FCFS --in=(file name) or type ./sched --policy=RR --quantum=(number) --in=(file name) to get the program to run and print the desired results. typing 'make clean' will get rid of the output files. To get the correct file name follow the format 'Workload_Samples/(file)'
